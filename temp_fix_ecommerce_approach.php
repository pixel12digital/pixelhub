...
echo "=== CORRIGINDO ABORDAGEM AUTOMÁTICA NAS PROPOSTAS ECOMMERCE ===\n\n";
...
if ($current) {
    ...
    // 2. Cria novo system prompt com abordagem inteligente
    $newPrompt = <<<PROMPT
...
PROMPT;
    ...
    echo "2. ATUALIZANDO CONTEXTO COM ABORDAGEM INTELIGENTE:\n";
    $stmt = $db->prepare('
        UPDATE ai_contexts 
    ...
    try {
        ...
        // 4. Verificação
        echo "3. VERIFICAÇÃO DAS ATUALIZAÇÕES:\n";
        $stmt = $db->prepare('SELECT system_prompt FROM ai_contexts WHERE slug = "ecommerce"');
        $stmt->execute();
        ...
        if ($updated) {
            ...
            $checks = [
                'ABORDAGEM INTELIGENTE - NUNCA USAR' => strpos($content, 'ABORDAGEM INTELIGENTE - NUNCA USAR') !== false,
                'Oi, entendi que você quer' => strpos($content, 'Oi, entendi que você quer') !== false,
                'Oi, vi que você tem' => strpos($content, 'Oi, vi que você tem') !== false,
                'Nossa loja virtual é perfeita' => strpos($content, 'Nossa loja virtual é perfeita') !== false,
                'ABORDAGEM INTELIGENTE - SEMPRE USAR' => strpos($content, 'ABORDAGEM INTELIGENTE - SEMPRE USAR') !== false,
                'Abertura direta com benefício' => strpos($content, 'Abertura direta com benefício') !== false,
                'entregas programadas' => strpos($content, 'entregas programadas') !== false,
                'métodos de envio' => strpos($content, 'métodos de envio') !== false,
                'vender pelo WhatsApp' => strpos($content, 'vender pelo WhatsApp') !== false,
                'agendamentos automáticos' => strpos($content, 'agendamentos automáticos') !== false,
                'cálculo automático de frete' => strpos($content, 'cálculo automático de frete') !== false
            ];
            
            echo "Verificação de conteúdo:\n";
            foreach ($checks as $check => $found) {
                if (strpos($check, 'NUNCA USAR') !== false) {
                    $status = $found ? '✅' : '❌';
                } else {
                    $status = $found ? '✅' : '❌';
                }
    ...
    // 5. Cria exemplos inteligentes
    echo "\n4. CRIANDO EXEMPLOS INTELIGENTES:\n";
    
    $intelligentExamples = [
        [
            'situation' => 'Proposta E-commerce - Alimentos - Abertura inteligente',
...
Quer saber como funciona a entrega programada?'
        ...
        [
            'situation' => 'Proposta E-commerce - Moda - Abertura inteligente',
...
Posso mostrar como integra?'
        ...
        [
            'situation' => 'Proposta E-commerce - Serviços - Abertura inteligente',
...
Quer testar o agendamento automático?'
    ...
    // Insere exemplos inteligentes
    foreach ($intelligentExamples as $i => $example) {
        $stmt = $db->prepare('
        ...
        try {
            ...
            $id = $db->lastInsertId();
            echo "✅ Exemplo inteligente " . ($i + 1) . " criado com ID: {$id}\n";
        } catch (Exception $e) {
...
echo "✅ Abordagens automáticas eliminadas\n";
echo "✅ Aberturas diretas com benefícios implementadas\n";
echo "✅ Inteligência contextual por segmento\n";
echo "✅ Foco em métodos de envio específicos\n";
echo "✅ Propostas mais naturais e inteligentes\n";
echo "✅ Exemplos práticos criados\n\n";

echo "🎯 RESULTADO ESPERADO:\n";
echo "- Sem mais \"Oi, entendi que você quer...\"\n";
echo "- Sem mais \"Oi, vi que você tem...\"\n";
echo "- Abertura direta com benefício específico\n";
echo "- Inteligência contextual do segmento\n";
echo "- Propostas que soam naturais e pensadas\n";
