 ...
 * 
 * EXECUÇÃO:
 * php scripts/migrate_leads_to_tenants_env_final.php [--dry-run] [--force]
 * 
 * OPTIONS:
...
class LeadMigration
{
    ...
    private function showCurrentStatus(): void
    {
        ...
        echo "- Leads restantes na tabela leads: {$leadsRemaining}\n";
        echo "- Leads já migrados como tenants: {$leadsMigrated}\n";
    ...
    private function performMigration(int $totalLeads): void
    {
        ...
        while ($offset < $totalLeads) {
            ...
            foreach ($leads as $lead) {
                try {
                    // Verifica se o lead tem nome válido
                    if (empty($lead['name']) || trim($lead['name']) === '') {
                        echo "⚠️  Lead #{$lead['id']} ignorado: nome em branco\n";
                        $skipped++;
                        continue;
                    }
