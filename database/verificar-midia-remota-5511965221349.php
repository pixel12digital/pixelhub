...
if (empty($foundEvents)) {
    ...
    echo "   ✅ Encontrados " . count($foundEvents) . " eventos únicos\n\n";
}
...
foreach ($foundEvents as $event) {
    ...
    // Verifica se tem mídia pelo tipo
    $type = $payload['type'] 
...
// 3. Mostra detalhes das mídias processadas
...
if (!empty($unprocessedMedia)) {
    ...
    foreach (array_slice($unprocessedMedia, 0, 5) as $i => $event) {
        ...
        echo "   ⚠️  Mídia " . ($i + 1) . " (não processada):\n";
        echo "      - Event ID: {$event['event_id']}\n";
        echo "      - Data: {$event['created_at']}\n";
        echo "      - Tipo: {$type}\n";
        echo "      - From: {$from}\n";
...
// 5. Busca direta na tabela communication_media via JOIN
...
$directMedia = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
echo "RESUMO FINAL\n";
echo "========================================\n";
echo "Total de eventos encontrados: " . count($foundEvents) . "\n";
