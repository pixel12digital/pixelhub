...
/**
 * Script para executar as migrations do tracking system
 * Usa conexão remota através das credenciais do .env
 */
...
try {
    ...
    echo "=== Implementação Tracking System ===\n\n";
    ...
    echo "   Colunas encontradas em migrations: " . implode(', ', $columns) . "\n";
    
    // Determinar coluna principal
    if (in_array('name', $columns)) {
        $mainColumn = 'name';
    } elseif (in_array('migration', $columns)) {
        $mainColumn = 'migration';
    } elseif (in_array('migration_name', $columns)) {
        $mainColumn = 'migration_name';
    } elseif (in_array('filename', $columns)) {
        $mainColumn = 'filename';
    } else {
        echo "   ⚠️  Nenhuma coluna padrão encontrada, pulando registro de migrations\n";
        $mainColumn = null;
    }
    
    if ($mainColumn) {
        // Ajustar nome da coluna de data também
        $dateColumn = in_array('executed_at', $columns) ? 'executed_at' : 
                     (in_array('run_at', $columns) ? 'run_at' : 'executed_at');
        
        $db->exec("
            INSERT INTO migrations ({$mainColumn}, {$dateColumn}) VALUES 
            ('20260220_create_tracking_codes_table', NOW()),
            ('20260220_add_tracking_fields_to_leads_and_opportunities', NOW())
            ON DUPLICATE KEY UPDATE {$dateColumn} = NOW()
        ");
        echo "   ✅ Migrations registradas usando coluna '{$mainColumn}' e data '{$dateColumn}'\n";
    }
    echo "\n";
