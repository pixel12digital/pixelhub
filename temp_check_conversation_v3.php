...
$phone = '+555599235045';
$sql = "SELECT c.id, c.conversation_key, c.contact_external_id, c.contact_name, c.tenant_id, c.status, c.created_at, c.last_activity_at, 
...
if (empty($conversations)) {
    ...
    $sql2 = "SELECT ce.id, ce.event_type, ce.contact_external_id, ce.tenant_id, ce.created_at,
                    t.name as tenant_name,
                    JSON_EXTRACT(ce.raw_data, '$.message.body') as message_body
             FROM communication_events ce 
             JOIN tenants t ON ce.tenant_id = t.id 
             WHERE ce.contact_external_id LIKE ? OR ce.raw_data LIKE ?
    ...
    if (empty($events)) {
        ...
        foreach ($events as $event) {
            echo "ID: {$event['id']} | Tipo: {$event['event_type']} | Contact: {$event['contact_external_id']} | Tenant: {$event['tenant_name']} | Data: {$event['created_at']}\n";
            echo "Mensagem: " . substr($event['message_body'] ?: 'N/A', 0, 100) . "\n\n";
        }
    }
} else {
    foreach ($conversations as $conv) {
        echo "ID: {$conv['id']} | Key: {$conv['conversation_key']} | Contact: {$conv['contact_external_id']} | Nome: {$conv['contact_name']} | Tenant: {$conv['tenant_name']} | Status: {$conv['status']} | Última atividade: {$conv['last_activity_at']}\n\n";
    }
}
