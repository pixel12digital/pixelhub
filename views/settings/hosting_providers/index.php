...
<div class="content-header">
    <h2>Provedores de Hospedagem</h2>
    <p style="color: #666; font-size: 14px; margin-top: 5px;">
        Gerencie os provedores de hospedagem disponíveis para o campo "Provedor Atual" das contas de hospedagem.
    </p>
</div>
...
<?php if (isset($_GET['success'])): ?>
    <div class="card" style="background: #d4edda; border-left: 4px solid #28a745; margin-bottom: 20px;">
        <p style="color: #155724; margin: 0;">
            ...
            if ($success === 'created') echo 'Provedor criado com sucesso.';
            elseif ($success === 'updated') echo 'Provedor atualizado com sucesso.';
            elseif ($success === 'toggled') echo 'Status do provedor alterado com sucesso.';
            ?>
        </p>
    </div>
<?php endif; ?>
...
<?php if (isset($_GET['error'])): ?>
    <div class="card" style="background: #fee; border-left: 4px solid #c33; margin-bottom: 20px;">
        <p style="color: #c33; margin: 0;">
            <?php
            $error = $_GET['error'];
            if ($error === 'not_found') echo 'Provedor não encontrado.';
...
<div style="margin-bottom: 20px;">
    <a href="<?= pixelhub_url('/settings/hosting-providers/create') ?>" 
...
<div class="card">
    <?php if (empty($providers)): ?>
        <div style="padding: 40px; text-align: center; color: #6c757d;">
            <p style="font-size: 16px; margin-bottom: 10px;">Nenhum provedor cadastrado.</p>
            <p style="font-size: 14px;">Crie o primeiro provedor para começar.</p>
        </div>
    <?php else: ?>
                ...
                <?php foreach ($providers as $provider): ?>
                    <tr style="border-bottom: 1px solid #dee2e6;">
                        ...
                        <td style="padding: 12px; text-align: center;">
                            <span class="badge <?= $provider['is_active'] ? 'badge-success' : 'badge-default' ?>">
                                <?= $provider['is_active'] ? 'Ativo' : 'Inativo' ?>
                            </span>
                        </td>
                        <td style="padding: 12px; text-align: center; color: #6c757d;">
                            <?= (int) $provider['sort_order'] ?>
                        </td>
                        <td style="padding: 12px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <a href="<?= pixelhub_url('/settings/hosting-providers/edit?id=' . $provider['id']) ?>" 
                                   style="color: #023A8D; text-decoration: none; font-size: 13px; font-weight: 500;">
                                    Editar
                                </a>
                                <form method="POST" action="<?= pixelhub_url('/settings/hosting-providers/toggle-status') ?>" 
...
$title = 'Provedores de Hospedagem';
