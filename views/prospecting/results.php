...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;">
    <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
        <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:800px;">
            ...
            <tbody>
                <?php $rowIdx = 0; foreach ($results as $result):
                    $rowIdx++;
                    $st = $result['status'];
                ...
                <tr style="border-bottom:1px solid #e8edf2;<?= $st === 'discarded' ? 'opacity:.4;background:#f8fafc;filter:grayscale(.5);' : (!empty($result['whatsapp_sent_at']) ? 'background:#dbeafe;border-left:3px solid #60a5fa;' : 'background:#fff;border-left:3px solid transparent;') ?>" id="row-<?= $result['id'] ?>" data-status="<?= htmlspecialchars($st) ?>" data-name="<?= htmlspecialchars($result['name'] ?? '') ?>" data-wa-sent="<?= !empty($result['whatsapp_sent_at']) ? '1' : '0' ?>">
                    ...
                    <td style="padding:14px 16px;" id="phone-cell-<?= $result['id'] ?>">
                        ...
                        <?php if (!empty($phone)):
                            ...
                            if (strlen($phoneDigits)>=12 && substr($phoneDigits,0,2)==='55') $phoneDigits=substr($phoneDigits,2);
                            $isLandlinePhone = strlen($phoneDigits)>=3 && in_array($phoneDigits[2],['2','3','4','5']);
                        ?>
                        <div style="display:flex;align-items:center;gap:5px;">
                            <div id="phone-display-<?= $result['id'] ?>" style="font-size:12px;color:#374151;font-weight:500;"><?= htmlspecialchars($phone) ?></div>
                            <?php if ($isLandlinePhone): ?>
                            <span style="padding:1px 5px;background:#fef3c7;color:#92400e;border-radius:3px;font-size:9px;font-weight:700;" title="Número fixo — sem WhatsApp">FIXO</span>
                            <?php else: ?>
                    ...
                    <td style="padding:14px 16px;text-align:center;">
                        ...
                        <span id="wa-badge-<?= $result['id'] ?>" class="wa-badge" style="font-size:10px;color:#15803d;font-weight:600;display:<?= !empty($result['whatsapp_sent_at']) ? 'flex' : 'none' ?>;align-items:center;gap:2px;" title="WA enviado<?= !empty($result['whatsapp_sent_at']) ? ' em '.date('d/m/Y H:i',strtotime($result['whatsapp_sent_at'])) : '' ?>"><svg width="10" height="10" viewBox="0 0 24 24" fill="#25d366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z"/></svg> WA enviado</span>
                        </div>
                    </td>
                    <td style="padding:14px 16px;text-align:center;">
                        <div style="display:flex;gap:4px;justify-content:center;align-items:center;flex-wrap:wrap;">
                            ...
                            $waDigits = preg_replace('/\D/','',$phoneForWA);
                            if (strlen($waDigits)>=12 && substr($waDigits,0,2)==='55') $waDigits=substr($waDigits,2);
                            $isLandlineWA = strlen($waDigits)>=3 && in_array($waDigits[2],['2','3','4','5']);
                            ?>
                            <!-- WhatsApp: sempre que tiver telefone -->
                            <?php if (!empty($phoneForWA)): ?>
                            <?php if ($isLandlineWA): ?>
                            <button disabled id="wa-btn-<?= $result['id'] ?>"
                                    style="padding:8px;background:#e5e7eb;color:#9ca3af;border:none;border-radius:6px;cursor:not-allowed;display:inline-flex;align-items:center;justify-content:center;min-height:36px;min-width:36px;"
                                    title="Número fixo — não tem WhatsApp">
        ...
        if (data.found && data.phone) {
            if (cell) {
                ...
                cell.innerHTML =
                    ...
                    '<button onclick="openProspectWA(\'' + phSafe + '\', \'\', ' + resultId + ')" title="Enviar WhatsApp" ' +
                    'style="padding:2px 5px;background:#25d366;color:#fff;border:none;border-radius:4px;font-size:10px;cursor:pointer;">WA</button>' +
            ...
            if (enrichBtn) {
                ...
                if (actionsDiv && !document.getElementById('wa-btn-' + resultId)) {
                    ...
                    waBtn.onclick = function() { openProspectWA(data.phone, '', resultId); };
    ...
    .then(data => {
        ...
        if (cell) {
            ...
            cell.innerHTML =
                ...
                '<button onclick="openProspectWA(\'' + phoneSafe + '\', \'\', ' + resultId + ')" title="Enviar WhatsApp" ' +
                'style="padding:2px 5px;background:#25d366;color:#fff;border:none;border-radius:4px;font-size:10px;cursor:pointer;">WA</button>' +
        ...
        if (actWA) {
            actWA.onclick = function() { openProspectWA(phone, '', resultId); };
        } else {
            const enrichBtn = document.getElementById('enrich-btn-' + resultId);
            ...
            if (actionsDiv) {
                ...
                waBtn.title = 'Enviar mensagem WhatsApp';
                waBtn.style.cssText = 'padding:6px;background:#25d366;color:#fff;border:none;border-radius:4px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;';
                waBtn.onclick = function() { openProspectWA(phone, '', resultId); };
                waBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z"/></svg>';
                actionsDiv.insertBefore(waBtn, actionsDiv.firstChild);
            }
        }
...
function openProspectWA(phone, name, resultId) {
    if (!name && resultId) {
                ...
                if (data.found && data.phone) {
                    ...
                    if (cell) {
                        ...
                        cell.innerHTML =
                            ...
                            '<button onclick="openProspectWA(\'' + phS + '\',\'\', ' + id + ')" title="Enviar WhatsApp" style="padding:2px 5px;background:#25d366;color:#fff;border:none;border-radius:4px;font-size:10px;cursor:pointer;">WA</button>' +
