    ...
    <form method="POST" action="<?= pixelhub_url($emailAccount ? '/email-accounts/update' : '/email-accounts/store') ?>">
        ...
        <div style="margin-bottom: 20px; padding: 15px; background: #f9f9f9; border-radius: 4px; border-left: 4px solid #023A8D;">
            ...
            <div style="margin-bottom: 15px;">
                ...
                <select id="provider" name="provider" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;"><option value="">Selecione um provedor...</option><?php if (!empty($providers)): ?><?php foreach ($providers as $provider): ?><option value="<?= htmlspecialchars($provider['slug']) ?>" <?= ($emailAccount && ($emailAccount['provider'] ?? '') === $provider['slug']) ? 'selected' : ' ?>><?= htmlspecialchars($provider['name']) ?></option><?php endforeach; ?><?php else: ?><option value="google_workspace">Google Workspace</option><option value="microsoft_365">Microsoft 365</option><option value="cpanel">cPanel</option><option value="outro">Outro</option><?php endif; ?></select> 
