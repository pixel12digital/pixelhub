...
if (empty($configs)) {
    echo "  ❌ NENHUMA configuração meta_official encontrada!\n";
} else {
    foreach ($configs as $c) {
        echo "  ID={$c['id']} phone_number_id={$c['meta_phone_number_id']} business_account={$c['meta_business_account_id']} active={$c['is_active']} global={$c['is_global']} token_preview={$c['token_preview']}...\n";
    }
}

echo "\n2) TEMPLATES APROVADOS (whatsapp_message_templates):\n";
$stmt = $db->query("SELECT id, template_name, status, language, is_active FROM whatsapp_message_templates WHERE status = 'approved' LIMIT 10");
$templates = $stmt->fetchAll(PDO::FETCH_ASSOC);
if (empty($templates)) {
    echo "  ❌ NENHUM template com status='approved' encontrado!\n";
    echo "  → Verificando todos os status:\n";
    $stmt2 = $db->query("SELECT id, template_name, status, language FROM whatsapp_message_templates LIMIT 10");
    foreach ($stmt2->fetchAll(PDO::FETCH_ASSOC) as $t) {
        echo "    ID={$t['id']} name={$t['template_name']} status={$t['status']} lang={$t['language']}\n";
    }
} else {
...
if (!$config) {
    echo "\n❌ Config Meta ativa + global não encontrada! Não é possível enviar.\n";
    echo '</pre>';
    exit;
}
...
if (strpos($accessToken, 'encrypted:') === 0) {
    ...
    echo "\n3) Token: não criptografado (primeiros 20 chars: " . substr($accessToken, 0, 20) . "...)\n";
}

$phoneNumberId = $config['meta_phone_number_id'];
echo "   phone_number_id: {$phoneNumberId}\n";

// 3. Normaliza telefone de teste
$testPhone = '4796164699';
$normalized = PhoneNormalizer::toE164OrNull($testPhone);
echo "\n4) Normalização de telefone:\n";
echo "   Input: {$testPhone}\n";
echo "   Resultado: " . ($normalized ?: 'NULL (inválido!)') . "\n";

// 4. Testa conexão com Meta API (busca info do número)
echo "\n5) TESTE DE CONEXÃO - Verificar phone number ID no Meta:\n";
...
// 5. Busca templates do Meta (lista real)
echo "\n6) TEMPLATES DISPONÍVEIS NO META API:\n";
$businessAccountId = $config['meta_business_account_id'];
$templatesUrl = "https://graph.facebook.com/v18.0/{$businessAccountId}/message_templates?fields=name,status,language&limit=10";
...
// 6. Tenta envio real para número de teste
if ($normalized && !empty($templates)) {
